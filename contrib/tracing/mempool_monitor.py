...
""" Example logging Dash Core mempool events using the mempool:added,
    mempool:removed, and mempool:rejected tracepoints. """
...
def main(dashd_path):
    dashd_with_usdts = USDT(path=str(dashd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    dashd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    dashd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    dashd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[dashd_with_usdts])
...
class Dashboard:
    ...
    def __init__(self, screen):
        ...
        self._timestamps = {"added": [], "removed": [], "rejected": []}
        self._event_history = {"added": 0, "removed": 0, "rejected": 0}
        self._init_windows()
    ...
    def _update_event_count(self, count):
        ...
        for line, metric in enumerate(["added", "removed", "rejected"]):
