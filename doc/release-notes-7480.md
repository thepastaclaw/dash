...
* After a mixing session completes successfully, the inputs a client
  ...
  spending them. If the wallet still has not observed that spend after an hour,
  CoinJoin starts rechecking the chain and mempool before deciding whether the
  finalized transaction never propagated. Previously the completion message
  routinely overtook the trickle-relayed mixing transaction, leaving a window
  in which another session (with
