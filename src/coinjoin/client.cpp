...
bool CCoinJoinClientSession::SignFinalTransaction(CNode& peer, CChainState& active_chainstate, CConnman& connman, const CTxMemPool& mempool, const CTransaction& finalTransactionNew)
{
    ...
    if (!IsValidInOuts(active_chainstate, m_isman, mempool, finalMutableTransaction.vin, finalMutableTransaction.vout,
                       nMessageID, nullptr, /*fFinalTx=*/true)) {
...
bool CCoinJoinClientSession::SelectRebalanceInputs(int nTargetDenom, bool fPromotion, std::vector<CTxDSIn>& vecTxDSInRet)
{
    ...
    if (fPromotion) {
        ...
        // Demotion: select 1 coin of the larger adjacent denomination.
...
bool CCoinJoinClientManager::ShouldPromote(int nSmallerDenom, int nLargerDenom, const wallet::CoinJoinDenomCounts& counts) const
{
    ...
    const int nSmallerCount = counts.total[idxSmaller];
    const int nLargerCount = counts.total[idxLarger];
    ...
    const int nFullyMixedCount = counts.fully_mixed[idxSmaller];
...
bool CCoinJoinClientManager::ShouldDemote(int nLargerDenom, int nSmallerDenom, const wallet::CoinJoinDenomCounts& counts) const
{
    // Validate denominations are adjacent
