...
bool CCoinJoinBaseSession::IsValidInOuts(CChainState& active_chainstate, const llmq::CInstantSendManager& isman,
                                         ...
                                         bool* fConsumeCollateralRet, bool fFinalTx) const
{
    ...
    // 0 (never matched) for per-entry validation or when nSessionDenom is the largest denom.
    int nLargerDenom{0};
    ...
    if (entryType == EntryType::PROMOTION) {
        ...
        nLargerDenom = CoinJoin::GetLargerAdjacentDenom(nSessionDenom);
    }
