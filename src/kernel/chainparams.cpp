...
static CBlock FindDevNetGenesisBlock(const CBlock &prevBlock, const std::string& devNetName, const CAmount& reward)
{
    assert(!devNetName.empty());
...
public:
    CMainParams() {
        ...
        consensus.vDeployments[Consensus::DEPLOYMENT_V24].bit = 13;
...
public:
    CTestNetParams() {
        ...
        consensus.vDeployments[Consensus::DEPLOYMENT_V24].bit = 13;
        consensus.vDeployments[Consensus::DEPLOYMENT_V24].nStartTime = 1790208000;     // Thursday, September 24, 2026 0:00:00
...
public:
    explicit CDevNetParams(const DevNetOptions& opts) {
        ...
        UpdateDevnetSubsidyAndDiffParameters(opts);
        genesis = CreateGenesisBlock(1417713337, 1096447, 0x207fffff, 1, 50 * COIN);
        consensus.hashGenesisBlock = genesis.GetHash();
        assert(consensus.hashGenesisBlock == uint256S("0x000008ca1832a4baf228eb1553c03d3a2c8e02399550dd6ea8d65cec3ef23d2e"));
        assert(genesis.hashMerkleRoot == uint256S("0xe0028eb9648db56b1ac77cf090b99048a8007e2bb64b68f092c03c7f56a662c7"));

        devnetGenesis = FindDevNetGenesisBlock(genesis, opts.name, 50 * COIN);
        ...
        UpdateDevnetLLMQChainLocks(opts);
        UpdateDevnetLLMQDIP0024InstantSend(opts);
        UpdateDevnetLLMQPlatform(opts);
        UpdateDevnetLLMQMnhf(opts);
        UpdateLLMQDevnetParameters(opts);
        UpdateDevnetPowTargetSpacing(opts);
...
private:
     ...
     */
    void UpdateDevnetSubsidyAndDiffParameters(const DevNetOptions& opts)
    {
        if (opts.minimum_difficulty_blocks) consensus.nMinimumDifficultyBlocks = *opts.minimum_difficulty_blocks;
        if (opts.high_subsidy_blocks) consensus.nHighSubsidyBlocks = *opts.high_subsidy_blocks;
        if (opts.high_subsidy_factor) consensus.nHighSubsidyFactor = *opts.high_subsidy_factor;
    }

    /**
     * Allows modifying the LLMQ type for ChainLocks.
     */
    void UpdateDevnetLLMQChainLocks(const DevNetOptions& opts)
    {
        if (!opts.llmq_chainlocks) return;

        Consensus::LLMQType llmqType = Consensus::LLMQType::LLMQ_NONE;
        for (const auto& params : consensus.llmqs) {
            if (params.name == *opts.llmq_chainlocks) {
     ...
     */
    void UpdateDevnetLLMQDIP0024InstantSend(const DevNetOptions& opts)
    {
        if (!opts.llmq_dip0024_instantsend) return;

        Consensus::LLMQType llmqType = Consensus::LLMQType::LLMQ_NONE;
        for (const auto& params : consensus.llmqs) {
            if (params.name == *opts.llmq_dip0024_instantsend) {
                if (!params.useRotation) {
     ...
     */
    void UpdateDevnetLLMQPlatform(const DevNetOptions& opts)
    {
        if (!opts.llmq_platform) return;

        Consensus::LLMQType llmqType = Consensus::LLMQType::LLMQ_NONE;
        for (const auto& params : consensus.llmqs) {
            if (params.name == *opts.llmq_platform) {
                llmqType = params.type;
            }
        }
        if (llmqType == Consensus::LLMQType::LLMQ_NONE) {
     ...
     */
    void UpdateDevnetLLMQMnhf(const DevNetOptions& opts)
    {
        if (!opts.llmq_mnhf) return;

        Consensus::LLMQType llmqType = Consensus::LLMQType::LLMQ_NONE;
        for (const auto& params : consensus.llmqs) {
            if (params.name == *opts.llmq_mnhf) {
                llmqType = params.type;
            }
        }
        if (llmqType == Consensus::LLMQType::LLMQ_NONE) {
     ...
     */
    void UpdateDevnetPowTargetSpacing(const DevNetOptions& opts)
    {
        if (!opts.pow_target_spacing) return;
     ...
     */
    void UpdateLLMQDevnetParameters(const DevNetOptions& opts)
    {
        if (!opts.llmq_devnet_parameters) return;
        ...
        params->size = opts.llmq_devnet_parameters->size;
        params->minSize = opts.llmq_devnet_parameters->threshold;
        params->threshold = opts.llmq_devnet_parameters->threshold;
        params->dkgBadVotesThreshold = opts.llmq_devnet_parameters->threshold;
    }
};
...
public:
    explicit CRegTestParams(const RegTestOptions& opts) {
        ...
        UpdateActivationParameters(opts);
        UpdateDIP3Parameters(opts);
        UpdateBudgetParameters(opts);
        ...
        UpdateLLMQTestParameters(opts, Consensus::LLMQType::LLMQ_TEST);
        UpdateLLMQTestParameters(opts, Consensus::LLMQType::LLMQ_TEST_INSTANTSEND);
        UpdateLLMQTestParameters(opts, Consensus::LLMQType::LLMQ_TEST_PLATFORM);
        UpdateLLMQInstantSendDIP0024(opts);
...
private:
    /**
     * Allows modifying the buried activation heights and the Version Bits regtest parameters.
     */
    void UpdateActivationParameters(const RegTestOptions& opts)
    {
        for (const auto& [dep, height] : opts.activation_heights) {
            ...
            case Consensus::BuriedDeployment::DEPLOYMENT_CLTV:
                ...
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_BIP147:
                consensus.BIP147Height = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_CSV:
                consensus.CSVHeight = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_DIP0001:
                consensus.DIP0001Height = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_DIP0003:
                consensus.DIP0003Height = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_DIP0008:
                consensus.DIP0008Height = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_DIP0020:
                consensus.DIP0020Height = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_DIP0024:
                consensus.DIP0024Height = int{height};
                consensus.DIP0024QuorumsHeight = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_BRR:
                consensus.BRRHeight = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_V19:
                consensus.V19Height = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_V20:
                consensus.V20Height = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_MN_RR:
                consensus.MN_RRHeight = int{height};
                break;
            case Consensus::BuriedDeployment::DEPLOYMENT_WITHDRAWALS:
        ...
        for (const auto& [deployment_pos, version_bits_params] : opts.version_bits_parameters) {
            ...
            if (version_bits_params.window_size != -1) {
                consensus.vDeployments[deployment_pos].nWindowSize = version_bits_params.window_size;
            }
            if (version_bits_params.threshold_start != -1) {
                consensus.vDeployments[deployment_pos].nThresholdStart = version_bits_params.threshold_start;
            }
            if (version_bits_params.threshold_min != -1) {
                consensus.vDeployments[deployment_pos].nThresholdMin = version_bits_params.threshold_min;
            }
            if (version_bits_params.falloff_coeff != -1) {
                consensus.vDeployments[deployment_pos].nFalloffCoeff = version_bits_params.falloff_coeff;
            }
            if (version_bits_params.use_ehf != -1) {
                consensus.vDeployments[deployment_pos].useEHF = version_bits_params.use_ehf > 0;
            }
        }
    }
     ...
     */
    void UpdateDIP3Parameters(const RegTestOptions& opts)
    {
        if (!opts.dip3_parameters) return;

        consensus.DIP0003Height = opts.dip3_parameters->activation_height;
        consensus.DIP0003EnforcementHeight = opts.dip3_parameters->enforcement_height;
    }

    /**
     * Allows modifying the budget regtest parameters.
     */
    void UpdateBudgetParameters(const RegTestOptions& opts)
    {
        if (!opts.budget_parameters) return;

        consensus.nMasternodePaymentsStartBlock = opts.budget_parameters->masternode_payments_start_block;
        consensus.nBudgetPaymentsStartBlock = opts.budget_parameters->budget_payments_start_block;
        consensus.nSuperblockStartBlock = opts.budget_parameters->superblock_start_block;
    }

    /**
     * Allows modifying parameters of the test LLMQ
     */
    void UpdateLLMQTestParameters(const RegTestOptions& opts, const Consensus::LLMQType llmqType)
    {
        ...
        const auto it = opts.llmq_test_parameters.find(llmqType);
        if (it == opts.llmq_test_parameters.end()) return;

        auto params = std::ranges::find_if(consensus.llmqs, [llmqType](const auto& llmq){ return llmq.type == llmqType;});
        assert(params != consensus.llmqs.end());
        params->size = it->second.size;
        params->minSize = it->second.threshold;
        params->threshold = it->second.threshold;
        params->dkgBadVotesThreshold = it->second.threshold;
    }

    /**
     * Allows modifying the LLMQ type for InstantSend (DIP0024).
     */
    void UpdateLLMQInstantSendDIP0024(const RegTestOptions& opts)
    {
        if (!opts.llmq_dip0024_instantsend) return;
...
std::unique_ptr<const CChainParams> CChainParams::DevNet(const DevNetOptions& options)
{
    return std::make_unique<const CDevNetParams>(options);
}
