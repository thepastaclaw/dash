...
bool HasPreviousQuorumQuarterSnapshots(const Consensus::LLMQParams& llmqParams,
                                       const llmq::UtilParameters& util_params,
                                       gsl::not_null<const CBlockIndex*> pWorkBlockIndex,
                                       int cycleBaseHeight)
{
    const int cycleLength = llmqParams.dkgInterval;
    for (const int idx : util::irange(3)) {
        const CBlockIndex* const pCycleIndex{pWorkBlockIndex->GetAncestor(cycleBaseHeight -
                                                                          (cycleLength * (idx + 1)))};
        if (pCycleIndex == nullptr ||
            !util_params.m_qsnapman.GetSnapshotForBlock(llmqParams.type, pCycleIndex).has_value()) {
            return false;
        }
    }
    return true;
    ...
    int quorumHeight)
{
    ...
    if (!HasPreviousQuorumQuarterSnapshots(llmq_params, util_params, pWorkBlockIndex, cycleBaseHeight)) {
        return std::nullopt;
    }
    ...
    if (quorumMembers.size() <= static_cast<size_t>(quorumIndex) ||
        quorumMembers[quorumIndex].size() != static_cast<size_t>(llmq_params.size)) {
        return std::nullopt;
    }
