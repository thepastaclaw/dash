...
public:
    ...
    /// Verify the hash signature and additionally require the exact 65-byte size, a compact
    /// recovery header byte in the canonical 27..34 range (the only values SignCompact produces)
    /// and a low-S value, making the signature bytes non-malleable by third parties.
