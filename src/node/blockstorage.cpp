...
static FlatFileSeq LegacyBlockFileSeq()
{
...
static FILE* LegacyOpenBlockFile(const FlatFilePos& pos, bool fReadOnly = false)
{
    return LegacyBlockFileSeq().Open(pos, fReadOnly);
}

static FlatFileSeq LegacyUndoFileSeq()
{
    return FlatFileSeq(gArgs.GetBlocksDirPath(), "rev", gArgs.GetBoolArg("-tinyblk", false) ? 0x10000 /* 64kb */ : UNDOFILE_CHUNK_SIZE);
}

static FILE* LegacyOpenUndoFile(const FlatFilePos& pos, bool fReadOnly = false)
{
    return LegacyUndoFileSeq().Open(pos, fReadOnly);
}

bool ReadBlockFromDisk(CBlock& block, const FlatFilePos& pos, const Consensus::Params& consensus_params)
{
    block.SetNull();

    CAutoFile filein(LegacyOpenBlockFile(pos, true), SER_DISK, CLIENT_VERSION);
    if (filein.IsNull()) {
...
bool ReadBlockFromDisk(CBlock& block, const CBlockIndex* pindex, const Consensus::Params& consensus_params)
{
    const FlatFilePos block_pos{WITH_LOCK(cs_main, return pindex->GetBlockPos())};

    if (!ReadBlockFromDisk(block, block_pos, consensus_params)) {
        return false;
    }
...
bool UndoReadFromDisk(CBlockUndo& blockundo, const CBlockIndex* pindex)
{
    ...
    CAutoFile filein(LegacyOpenUndoFile(pos, true), SER_DISK, CLIENT_VERSION);
    if (filein.IsNull()) {
...
{
    return FlatFileSeq(m_opts.blocks_dir, "rev", gArgs.GetBoolArg("-tinyblk", false) ? 0x10000 /* 64kb */ : UNDOFILE_CHUNK_SIZE);
}
