...
struct KeyParser {
    ...
    template<typename I> std::optional<Key> FromString(I begin, I end) const
    {
        assert(m_out);
        Key key = m_keys.size();
        auto pk = ParsePubkey(key, {&*begin, &*end}, ParseScriptContext::P2SH, *m_out, m_key_parsing_error);
        if (!pk) return {};
    ...
    template<typename I> std::optional<Key> FromPKBytes(I begin, I end) const
    {
        ...
        if (pubkey.IsValid()) {
            Key key = m_keys.size();
            m_keys.push_back(InferPubkey(pubkey, ParseScriptContext::P2SH, *m_in));
            return key;
        }
        return {};
    }
...
std::unique_ptr<DescriptorImpl> ParseScript(uint32_t& key_exp_index, Span<const char>& sp, ParseScriptContext ctx, FlatSigningProvider& out, std::string& error)
{
    ...
    // Process miniscript expressions.
    {
        ...
        if (node) {
            if (ctx != ParseScriptContext::P2SH) {
                error = "Miniscript expressions can only be used in sh";
                return nullptr;
            }
