...
static CMutableTransaction CreateProRegTx(const ChainstateManager& chainman, SimpleUTXOMap& utxos, int port,
                                          const CScript& scriptPayout, const CKey& coinbaseKey, CKey& ownerKeyRet,
                                          CBLSSecretKey& operatorKeyRet, uint16_t version = 0,
                                          const CBLSPublicKey* operatorPubKey = nullptr)
{
    ownerKeyRet.MakeNewKey(true);
    operatorKeyRet.MakeNewKey();

    CProRegTx proTx;
    proTx.nVersion = version ? version : ProTxVersion::GetMax(!bls::bls_legacy_scheme, /*is_extended_addr=*/false);
    ...
    proTx.pubKeyOperator.Set(operatorPubKey ? *operatorPubKey : operatorKeyRet.GetPublicKey(),
                             proTx.nVersion == ProTxVersion::LegacyBLS);
...
static CMutableTransaction CreateProUpServTx(const ChainstateManager& chainman, SimpleUTXOMap& utxos,
                                             const uint256& proTxHash, const CBLSSecretKey& operatorKey, int port,
                                             ...
                                             uint16_t version = 0)
{
    ...
    proTx.inputsHash = CalcTxInputsHash(CTransaction(tx));
    proTx.sig = operatorKey.Sign(::SerializeHash(proTx), proTx.nVersion == ProTxVersion::LegacyBLS);
    SetTxPayload(tx, proTx);
    SignTransaction(tx, spent, coinbaseKey);
...
static CMutableTransaction CreateProUpRegTx(const ChainstateManager& chainman, SimpleUTXOMap& utxos,
                                            const uint256& proTxHash, const CKey& mnKey,
                                            const CBLSPublicKey& pubKeyOperator, const CKeyID& keyIDVoting,
                                            const CScript& scriptPayout, const CKey& coinbaseKey, uint16_t version = 0,
                                            CAmount fee = 0)
{
    CProUpRegTx proTx;
    proTx.nVersion = version ? version : ProTxVersion::GetMax(!bls::bls_legacy_scheme, /*is_extended_addr=*/false);
    proTx.proTxHash = proTxHash;
    proTx.pubKeyOperator.Set(pubKeyOperator, proTx.nVersion == ProTxVersion::LegacyBLS);
...
#include <test/evo_deterministicmns_bls_fixture.h>

#define BLS_MIGRATION_TEST(name)            \
    BOOST_AUTO_TEST_CASE(name)              \
    {                                       \
        TestChainV24SignalBeforeV19Setup s; \
        BLSMigrationTest h{s};
#define BLS_PRE_V19_TEST(name)               \
    BOOST_AUTO_TEST_CASE(name)               \
    {                                        \
        TestChainV19BeforeActivationSetup s; \
        BLSMigrationTest h{s};
BLS_MIGRATION_TEST(migration_rejected_when_key_squatted)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV19();
const auto key = a.op.GetPublicKey();
h.Reg(ProTxVersion::BasicBLS, &key);
h.MineV24();
h.Check(h.UpServ(a, ProTxVersion::BasicBLS), false, "bad-protx-dup-key");
h.Check(h.UpReg(a, key, ProTxVersion::BasicBLS), false, "bad-protx-dup-key");
}
BLS_MIGRATION_TEST(proupserv_migrates_legacy)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV24();
auto tx = h.UpServ(a, ProTxVersion::BasicBLS);
h.Check(tx, true);
h.Block({tx});
auto live = h.dmnman.GetListAtChainTip();
h.State(live, a, ProTxVersion::BasicBLS, a.op.GetPublicKey(), false);
h.State(h.Reload(live), a, ProTxVersion::BasicBLS, a.op.GetPublicKey(), false);
}
BLS_MIGRATION_TEST(same_mn_same_block_version_crossing_key_rotation)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV24();
auto k1 = h.Key(), k2 = h.Key();
auto t1 = h.UpReg(a, k1.GetPublicKey(), ProTxVersion::BasicBLS),
     t2 = h.UpReg(a, k2.GetPublicKey(), ProTxVersion::LegacyBLS);
h.Check(t1, true);
h.Check(t2, true);
BlockValidationState st;
CDeterministicMNList list;
if (h.Rebuild({t1, t2}, st, list)) {
    h.State(list, a, ProTxVersion::BasicBLS, k2.GetPublicKey(), true);
    h.State(h.Reload(list), a, ProTxVersion::BasicBLS, k2.GetPublicKey(), true);
}
}
BLS_MIGRATION_TEST(same_mn_same_block_migration_consistent)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV24();
auto key = h.Key();
auto t1 = h.UpReg(a, key.GetPublicKey(), ProTxVersion::LegacyBLS),
     t2 = h.UpReg(a, key.GetPublicKey(), ProTxVersion::BasicBLS);
h.Check(t1, true);
h.Check(t2, true);
BlockValidationState st;
CDeterministicMNList list;
BOOST_REQUIRE(h.Rebuild({t1, t2}, st, list));
h.State(list, a, ProTxVersion::BasicBLS, key.GetPublicKey(), true);
h.State(h.Reload(list), a, ProTxVersion::BasicBLS, key.GetPublicKey(), true);
}
BLS_MIGRATION_TEST(pre_v24_cross_scheme_pair_cannot_become_resident)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV19();
auto b = h.Reg(ProTxVersion::BasicBLS);
auto key = h.Key();
auto ta = h.UpReg(a, key.GetPublicKey(), ProTxVersion::LegacyBLS, 100000),
     tb = h.UpReg(b, key.GetPublicKey(), ProTxVersion::BasicBLS, 100000);
h.Mempool(ta, true);
h.Mempool(tb, false, "protx-dup");
h.MineV24();
auto& pool = *Assert(s.m_node.mempool);
BOOST_REQUIRE(WITH_LOCK(pool.cs, return pool.exists(ta.GetHash())));
BOOST_CHECK(h.TemplateHas(ta.GetHash()));
}
BLS_MIGRATION_TEST(same_block_cross_scheme_key_pair_rejected)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV24();
auto b = h.Reg(ProTxVersion::BasicBLS);
auto key = h.Key();
auto t1 = h.UpReg(b, key.GetPublicKey(), ProTxVersion::BasicBLS),
     t2 = h.UpReg(a, key.GetPublicKey(), ProTxVersion::LegacyBLS);
h.Check(t1, true);
h.Check(t2, true);
BlockValidationState st;
CDeterministicMNList list;
BOOST_CHECK(!h.Rebuild({t1, t2}, st, list));
BOOST_CHECK_EQUAL(st.GetRejectReason(), "bad-protx-dup-key");
}
BLS_MIGRATION_TEST(mempool_rejects_cross_scheme_key_race)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV24();
auto b = h.Reg(ProTxVersion::BasicBLS);
auto key = h.Key();
auto first = h.UpReg(b, key.GetPublicKey(), ProTxVersion::BasicBLS, 100000),
     second = h.UpReg(a, key.GetPublicKey(), ProTxVersion::LegacyBLS, 100000);
h.Mempool(first, true);
h.Check(second, true);
h.Mempool(second, false, "protx-dup");
}
BLS_MIGRATION_TEST(proupreg_rejects_cross_scheme_key_reuse)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV24();
auto b = h.Reg(ProTxVersion::BasicBLS);
h.Check(h.UpReg(a, b.op.GetPublicKey(), ProTxVersion::LegacyBLS), false, "bad-protx-dup-key");
h.Check(h.UpReg(b, a.op.GetPublicKey(), ProTxVersion::BasicBLS), false, "bad-protx-dup-key");
h.Check(h.UpReg(b, b.op.GetPublicKey(), ProTxVersion::BasicBLS), true);
}
BLS_MIGRATION_TEST(proregtx_rejects_cross_scheme_key_reuse)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV19();
h.Check(h.ProReg(a.op.GetPublicKey(), ProTxVersion::BasicBLS), true);
h.MineV24();
h.Check(h.ProReg(a.op.GetPublicKey(), ProTxVersion::BasicBLS), false, "bad-protx-dup-key");
auto fresh = h.Key();
h.Check(h.ProReg(fresh.GetPublicKey(), ProTxVersion::LegacyBLS), false, "bad-protx-version-disallowed");
h.Check(h.ProReg(fresh.GetPublicKey(), ProTxVersion::BasicBLS), true);
}
BOOST_AUTO_TEST_CASE(statediff_captures_operator_key_reencoding)
{
    ...
    a.pubKeyOperator.Set(key.GetPublicKey(), true);
    a.netInfo = NetInfoInterface::MakeNetInfo(a.nVersion);
    CDeterministicMNState b{a};
    b.nVersion = ProTxVersion::BasicBLS;
    b.pubKeyOperator.Set(key.GetPublicKey(), false);
    CDeterministicMNStateDiff diff{a, b};
    CDataStream ds(SER_DISK, CLIENT_VERSION);
    ds << diff;
    CDeterministicMNStateDiff loaded;
    ds >> loaded;
    CDeterministicMNState out{a};
    loaded.ApplyToState(out);
    BOOST_CHECK_EQUAL(out.nVersion, ProTxVersion::BasicBLS);
    BOOST_CHECK(!out.pubKeyOperator.IsLegacy());
    BOOST_CHECK(out.pubKeyOperator.Get() == key.GetPublicKey());
    BOOST_CHECK(::SerializeHash(out.pubKeyOperator) == ::SerializeHash(b.pubKeyOperator));
}
BLS_PRE_V19_TEST(has_operator_key_under_any_scheme)
auto a = h.Reg(ProTxVersion::LegacyBLS);
const auto list = h.dmnman.GetListAtChainTip();
BOOST_REQUIRE(list.GetMN(a.hash)->pdmnState->pubKeyOperator.IsLegacy());
CBLSLazyPublicKey basic;
basic.Set(a.op.GetPublicKey(), false);
BOOST_CHECK(!list.HasUniqueProperty(basic));
BOOST_CHECK(list.HasOperatorKeyUnderAnyScheme(a.op.GetPublicKey(), uint256()));
BOOST_CHECK(!list.HasOperatorKeyUnderAnyScheme(a.op.GetPublicKey(), a.hash));
BOOST_CHECK(!list.HasOperatorKeyUnderAnyScheme(h.Key().GetPublicKey(), uint256()));
}
BLS_PRE_V19_TEST(pre_v24_behaviour_unchanged)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV19();
BOOST_REQUIRE(!h.V24Active());
BOOST_REQUIRE(!bls::bls_legacy_scheme.load());
auto serv = h.UpServ(a, ProTxVersion::BasicBLS);
h.Check(serv, true);
h.Block({serv});
h.Reload(h.dmnman.GetListAtChainTip());
auto reg = h.UpReg(a, a.op.GetPublicKey(), ProTxVersion::BasicBLS);
h.Check(reg, true);
h.Block({reg});
auto live = h.dmnman.GetListAtChainTip();
h.State(live, a, ProTxVersion::LegacyBLS, a.op.GetPublicKey(), false);
h.Reload(live);
}
BLS_MIGRATION_TEST(stale_special_tx_does_not_poison_template)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV24();
auto stale = h.ProReg(a.op.GetPublicKey(), ProTxVersion::BasicBLS);
h.Check(stale, false, "bad-protx-dup-key");
auto& pool = *Assert(s.m_node.mempool);
TestMemPoolEntryHelper entry;
{
    LOCK2(cs_main, pool.cs);
    pool.addUnchecked(entry.Fee(50000).Time(Now<NodeSeconds>()).FromTx(stale));
    BOOST_REQUIRE(pool.exists(stale.GetHash()));
}
BOOST_CHECK(!h.TemplateHas(stale.GetHash()));
}
BLS_MIGRATION_TEST(proupreg_migrates_legacy_same_key)
auto a = h.Reg(ProTxVersion::LegacyBLS);
h.MineV24();
auto same = h.UpReg(a, a.op.GetPublicKey(), ProTxVersion::BasicBLS);
h.Check(same, true);
h.Block({same});
auto live = h.dmnman.GetListAtChainTip();
h.State(live, a, ProTxVersion::BasicBLS, a.op.GetPublicKey(), false);
h.State(h.Reload(live), a, ProTxVersion::BasicBLS, a.op.GetPublicKey(), false);
auto key = h.Key();
auto rotated = h.UpReg(a, key.GetPublicKey(), ProTxVersion::BasicBLS);
h.Check(rotated, true);
h.Block({rotated});
live = h.dmnman.GetListAtChainTip();
h.State(live, a, ProTxVersion::BasicBLS, key.GetPublicKey(), true);
h.Reload(live);
}

#undef BLS_PRE_V19_TEST
#undef BLS_MIGRATION_TEST
