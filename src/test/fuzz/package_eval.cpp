...
void initialize_tx_pool()
{
    ...
    for (int i = 0; i < 2 * COINBASE_MATURITY; ++i) {
        COutPoint prevout{MineBlock(g_setup->m_node, P2SH_OP_TRUE)};
...
FUZZ_TARGET(tx_package_eval, .init = initialize_tx_pool)
{
    ...
    LIMITED_WHILE(fuzzed_data_provider.ConsumeBool(), 300)
    {
        ...
        while (txs.size() < num_txs) {
            ...
            const CTransactionRef tx = [&] {
                ...
                for (size_t i = 0; i < num_in; ++i) {
                    ...
                    const auto script_sig = CScript{} << STACK_ELEM_OP_TRUE;
                    CTxIn in;
                    in.prevout = outpoint;
                ...
                for (int i = 0; i < num_out; ++i) {
                    tx_mut.vout.emplace_back(amount_out, P2SH_OP_TRUE);
                }
        ...
        if (result_package.m_state.GetResult() != PackageValidationResult::PCKG_POLICY) {
            auto it = result_package.m_tx_results.find(txs.back()->GetHash());
