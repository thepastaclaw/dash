...
// Chain/ProRegTx plumbing below mirrors evo_deterministicmns_tests.cpp; only the parts
// needed to register a single masternode are duplicated here.
...
SimpleUTXOMap SelectUTXOs(const CChain& active_chain, SimpleUTXOMap& utxos, CAmount amount, CAmount& changeRet)
{
    changeRet = 0;
...
SimpleUTXOMap FundTransaction(const ChainstateManager& chainman, CMutableTransaction& tx, SimpleUTXOMap& utxos,
                              const CScript& scriptPayout, CAmount amount)
{
    CAmount change;
...
void SignTransaction(CMutableTransaction& tx, const SimpleUTXOMap& coins, const CKey& coinbaseKey)
{
...
CMutableTransaction CreateProRegTx(const ChainstateManager& chainman, SimpleUTXOMap& utxos, const CScript& scriptPayout,
                                   const CKey& coinbaseKey, CKey& ownerKeyRet, CBLSSecretKey& operatorKeyRet)
{
    ...
    BOOST_REQUIRE_EQUAL(proTx.netInfo->AddEntry(NetInfoPurpose::CORE_P2P, "1.1.1.1:1"), NetInfoStatus::Success);
...
struct GovernanceVoteSetup : public TestChainSetup {
    ...
    void SetUp()
    {
        ...
        MineBlocks(1);
        auto protx = CreateProRegTx(*m_node.chainman, utxos, payout_script(), coinbaseKey, mn_voting_key, mn_operator_key);
    ...
    uint256 ConfirmProposalCollateral(const uint256& govobj_hash)
    {
        CMutableTransaction tx;
        CAmount change{0};
        const auto spent = WITH_LOCK(::cs_main, return SelectUTXOs(m_node.chainman->ActiveChain(), utxos,
                                                                   GOVERNANCE_PROPOSAL_FEE_TX, change));
        for (const auto& input : spent) {
            tx.vin.emplace_back(input.first);
        }
        tx.vout.emplace_back(GOVERNANCE_PROPOSAL_FEE_TX, CScript() << OP_RETURN << ToByteVector(govobj_hash));
        if (change != 0) {
            // IsCollateralValid() rejects the fee tx unless every output is P2PKH or unspendable.
            tx.vout.emplace_back(change, payout_script());
        }
        SignTransaction(tx, spent, coinbaseKey);
...
BOOST_AUTO_TEST_CASE(orphan_vote_is_cached_and_applied_when_parent_arrives)
{
    ...
    // Guard: if the masternode or its signature were not recognized the rest of this test would
    // pass for the wrong reason (an unverifiable vote is cached as an orphan just the same).
