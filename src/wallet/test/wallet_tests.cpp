...
BOOST_FIXTURE_TEST_CASE(public_descriptor_outputs_are_watch_only, TestChain100Setup)
{
...

    {
        ...
        // Product path: signing descriptors plus a public-only contact chain.
        AddKey(wallet, coinbaseKey);
        BOOST_CHECK(!wallet.HaveWatchOnly());
        ...
        // Public-only descriptors in a signing wallet must surface as watch-only
        // for both IsMine and the wallet-level HaveWatchOnly path used by Qt
        // balances / getbalances.
        BOOST_CHECK(wallet.HaveWatchOnly());
        BOOST_CHECK_EQUAL(wallet.IsMine(GetScriptForDestination(PKHash{coinbaseKey.GetPubKey()})),
                          ISMINE_SPENDABLE);
        ...
        BOOST_CHECK_EQUAL(bal.m_watchonly_trusted, 0);
        BOOST_CHECK_EQUAL(bal.m_watchonly_untrusted_pending, COIN);
    }

    // Descriptor wallets with private keys disabled keep spendable semantics
    // and must not advertise a separate watch-only balance.
    {
        CWallet watch_wallet(m_node.chain.get(), m_node.coinjoin_loader.get(), "", m_args, CreateDummyWalletDatabase());
        FlatSigningProvider watch_provider;
        std::string watch_error;
        auto watch_desc{Parse("pkh(" + EncodeExtPubKey(contact_xpub) + "/*)", watch_provider, watch_error,
                              /*require_checksum=*/false)};
        ...
        WalletDescriptor wallet_descriptor(std::move(watch_desc), /*creation_time=*/0,
                                           /*range_start=*/0, /*range_end=*/10,
                                           /*next_index=*/0);
        BOOST_REQUIRE(watch_wallet.AddWalletDescriptor(wallet_descriptor, watch_provider, "watch-only wallet",
                                                       /*internal=*/false));

        CExtPubKey child;
        BOOST_REQUIRE(contact_xpub.Derive(child, 0));
        const CScript contact_script{GetScriptForDestination(PKHash{child.pubkey})};
        BOOST_CHECK_EQUAL(watch_wallet.IsMine(contact_script), ISMINE_SPENDABLE);
        BOOST_CHECK(!watch_wallet.HaveWatchOnly());
    }
}
