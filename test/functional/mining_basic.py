    ...
    def test_blockmintxfee_parameter(self):
        ...
        for blockmintxfee_sat_kvb in (DEFAULT_BLOCK_MIN_TX_FEE, 0, 50, 100, 500, 2500, 5000, 21000, 333333, 2500000):
            ...
            if blockmintxfee_btc_kvb > 0:
                lowerfee_btc_kvb = blockmintxfee_btc_kvb - Decimal(12)/COIN  # 0.012 sat/byte lower (>= 1 sat less fee for the 85-byte MiniWallet tx)
            ...
            block_template_txids = [tx['hash'] for tx in block_template['transactions']]
            self.generate(self.wallet, 1, sync_fun=self.no_op)
