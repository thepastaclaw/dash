...
MAX_SIZE = 0x02000000
oversized_inv_bits = MAX_SIZE    # ~4 MiB per vector<bool>
...
class QuorumDataMessagesTest(DashTestFramework):
    ...
    def run_test(self):
        ...
        def test_basics():
            ...
            # Raw counts are rejected before BLS element decode. MAX_SIZE + 1 also
            # verifies that CompactSize failures reach the same disconnect path.
            for bad_count in (1000, MAX_SIZE + 1):
                send_bad_qdata_expect_disconnect(craft_qdata_with_bad_section(
                    qdata_valid, bad_vvec=ser_compact_size(bad_count)))
                send_bad_qdata_expect_disconnect(craft_qdata_with_bad_section(
                    qdata_valid, bad_contribs=ser_compact_size(bad_count)))
            mn1.get_node(self).disconnect_p2ps()
            mn2.get_node(self).disconnect_p2ps()
