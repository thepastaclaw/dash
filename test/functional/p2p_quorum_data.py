...
class QuorumDataMessagesTest(DashTestFramework):
    ...
    def run_test(self):
        ...
        def test_basics():
            ...
            # - Vector whose declared CompactSize count vastly exceeds the runtime max, with
            #   no element bytes following. A correctly bounded node rejects the count before
            #   touching a single BLS G1 decode and scores the peer 100 (disconnect). A node
            #   that only checks size after decoding the whole vector would hit a stream
            #   underrun during element decoding, throw ios_base::failure, and leave the peer
            #   connected — that observable difference is what distinguishes pre-count
            #   rejection from generic truncated-data handling.
            #
